...
// Field modulus q
const (
	q0 uint64 = 10182971180934965931
	q1 uint64 = 15488787195747417982
	q2 uint64 = 1628721857945875526
	q3 uint64 = 17478405972920225849
	q4 uint64 = 1177913551803681068
)
...
const qInvNeg uint64 = 6176088765535387645

func init() {
	_modulus.SetString("1058ca226f60892cf28fc5a0b7f9d039169a61e684c73446d6f339e43424bf7e8d512e565dab2aab", 16)
}
...
func (z *Element) SetOne() *Element {
	z[0] = 13276128949361475579
	z[1] = 7475865022012901269
	z[2] = 12462660278230970329
	z[3] = 14525071511839886503
	z[4] = 778040796654335581
	return z
}
...
func (z *Element) IsOne() bool {
	return ((z[4] ^ 778040796654335581) | (z[3] ^ 14525071511839886503) | (z[2] ^ 12462660278230970329) | (z[1] ^ 7475865022012901269) | (z[0] ^ 13276128949361475579)) == 0
}
...
func (z *Element) LexicographicallyLargest() bool {
	...
	var b uint64
	_, b = bits.Sub64(_z[0], 5091485590467482966, 0)
	_, b = bits.Sub64(_z[1], 7744393597873708991, b)
	_, b = bits.Sub64(_z[2], 10037732965827713571, b)
	_, b = bits.Sub64(_z[3], 8739202986460112924, b)
	_, b = bits.Sub64(_z[4], 588956775901840534, b)
...
func (z *Element) SetRandom() (*Element, error) {
	...
	// l is number of limbs * 8; the number of bytes needed to reconstruct 5 uint64
	const l = 40
...
func (z *Element) smallerThanModulus() bool {
	return (z[4] < q4 || (z[4] == q4 && (z[3] < q3 || (z[3] == q3 && (z[2] < q2 || (z[2] == q2 && (z[1] < q1 || (z[1] == q1 && (z[0] < q0)))))))))
}
...
func (z *Element) Neg(x *Element) *Element {
	...
	z[4], _ = bits.Sub64(q4, x[4], borrow)
	return z
}
...
// Bits provides access to z by returning its value as a little-endian [5]uint64 array.
...
const (
	inversionCorrectionFactorWord0 = 13068848450370500644
	inversionCorrectionFactorWord1 = 8434285418056431308
	inversionCorrectionFactorWord2 = 8621508525510720840
	inversionCorrectionFactorWord3 = 10079858226894004022
	inversionCorrectionFactorWord4 = 237932516250676789
	invIterationsN                 = 22
)
...
func (z *Element) montReduceSigned(x *Element, xHi uint64) {
	...
	// m * qElement[4] ≤ (2⁶⁴ - 1) * (2⁶³ - 1) = 2¹²⁷ - 2⁶⁴ - 2⁶³ + 1
	// x[4] + C ≤ 2*(2⁶⁴ - 1) = 2⁶⁵ - 2
	// On LHS, (C, t[4]) ≤ 2¹²⁷ - 2⁶⁴ - 2⁶³ + 1 + 2⁶⁵ - 2 = 2¹²⁷ + 2⁶³ - 1
	// So on LHS, C ≤ 2⁶³
	t[5] = xHi + C
	// xHi + C < 2⁶³ + 2⁶³ = 2⁶⁴
	...
	if mustNeg {
		...
		if b != 0 {
			...
			z[2], carry = bits.Add64(z[2], q2, carry)
			z[3], carry = bits.Add64(z[3], q3, carry)
			z[4], _ = bits.Add64(neg1, q4, carry)
		}
	}
}
